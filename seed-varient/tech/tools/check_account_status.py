...
description: Fetch financial account status, customer balances, and P&L summary from ERPNext.
author: Quazi
version: 1.0.0
"""
...
class Tools:
    ...
    def _get(self, endpoint: str, params: dict = None) -> dict:
        resp = requests.get(
            f"{self.valves.ERPNEXT_URL}{endpoint}",
            params=params or {},
    ...
    def check_account_status(
        ...
        """
        Get financial account status from ERPNext.

        Use 'summary' for overall business health (income, expenses, receivables).
        Use 'customer' with a customer_name to check a specific customer's balance.

        :param report_type: 'summary' for P&L overview, or 'customer' for customer balance.
        :param customer_name: Required when report_type is 'customer'.
        :return: Financial data as formatted text.
        """
        try:
    ...
    def _financial_summary(self) -> str:
        invoices = self._get("/api/resource/Sales Invoice", {
            "filters": json.dumps([["docstatus", "=", 1]]),
            "fields": json.dumps(["sum(grand_total) as total_sales", "count(name) as count"]),
            "limit_page_length": 1,
        }).get("data", [{}])

        outstanding = self._get("/api/resource/Sales Invoice", {
            "filters": json.dumps([["docstatus", "=", 1], ["outstanding_amount", ">", 0]]),
            "fields": json.dumps(["sum(outstanding_amount) as total_outstanding", "count(name) as count"]),
            "limit_page_length": 1,
        }).get("data", [{}])
        ...
        return json.dumps({
            ...
            "total_outstanding": unpaid.get("total_outstanding", 0),
            "unpaid_invoices": unpaid.get("count", 0),
        })

    def _customer_balance(self, customer_name: str) -> str:
        data = self._get("/api/resource/Sales Invoice", {
            "filters": json.dumps([
        ...
        if not data:
            return json.dumps({
                "status": "not_found",
                "message": f"No invoices found for '{customer_name}'.",
            })
