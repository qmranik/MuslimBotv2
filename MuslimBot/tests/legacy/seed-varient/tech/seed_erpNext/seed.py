"""
Quazi-Tech ERPNext Seeder
=========================
Populates a fresh ERPNext instance with realistic computer store demo data.

Usage:
    pip install -r requirements.txt
    python seed.py --url http://localhost:8080 --api-key <KEY> --api-secret <SECRET>
...
class ERPNextSeeder:
    ...
    def _api(self, method: str, endpoint: str, data: dict = None) -> dict:
        url = f"{self.url}/api/resource/{endpoint}"
        try:
            if method == "GET":
                resp = self.session.get(url, params=data or {})
            else:
                resp = self.session.post(url, json={"data": json.dumps(data)} if data else None)
    ...
    def _get_company(self) -> str:
        resp = self._api("GET", "Company", {"limit_page_length": 1})
        companies = resp.get("data", [])
        if companies:
            return companies[0]["name"]
        print("ERROR: No company found. Set up ERPNext wizard first.")
        sys.exit(1)

    def _get_abbr(self, company: str) -> str:
        resp = self.session.get(f"{self.url}/api/resource/Company/{company}")
        return resp.json().get("data", {}).get("abbr", "QT")
    ...
    def seed_all(self):
        ...
        print("\n=== Seeding complete! ===")
        print(f"Login at {self.url} with your admin credentials.")

    def _seed_item_groups(self):
        print("--- Item Groups ---")
        groups = [
            "Laptops", "Desktops", "Monitors", "Components",
            "Storage", "Peripherals", "Power & Networking",
    ...
    def _seed_supplier_groups(self):
        ...
        for g in ["Brand Distributor", "Authorized Dealer"]:
        ...
        for item in items:
            if item.get("opening_qty", 0) > 0:
                entry["items"].append({
                    ...
                    "basic_rate": int(item["standard_rate"] * 0.7),
    ...
    def _seed_sales_invoices(self, company: str, abbr: str):
        ...
        for order in orders:
            ...
            if resp.status_code in (200, 201):
                ...
                print(f"  Created Sales Invoice: {si_name} ({order['customer']})")
                self.session.put(
...
def main():
    parser = argparse.ArgumentParser(description="Seed ERPNext with Quazi-Tech demo data")
    parser.add_argument("--url", default="http://localhost:8080", help="ERPNext URL")
    ...
    parser.add_argument("--user", default="", help="ERPNext username (alternative to API key)")
    parser.add_argument("--password", default="", help="ERPNext password")
